...
def plot_all_k_perms(in_files, stat_files):
    ...
    for permutation in results_by_permutation:
        ...
        fig, axs = plt.subplots(1, columns, figsize=(5 * columns, 5))
        if columns == 1:
            axs = [axs]
...
def plot_all_k_perms_hq(in_files, stat_files):
    results_by_permutation = dict()
    for file_idx, in_file in enumerate(in_files):
        with open(in_file) as f:
            results = json.load(f)
    ...
    for permutation in results_by_permutation:
        ...
        plt.suptitle("Core edge permutation \"%s\"" % permutation_name[0].upper())
        plt.tight_layout()
        out_file = "graph_edit_distance/k_perms_combined_hq_%s_%s.png" % (permutation_name[0], permutation_name[1])
        plt.savefig(out_file, dpi=300)
        plt.clf()
...
if __name__ == "__main__":
    ...
    plot_all_k_perms_hq(
        {
            "k_perms_copd1_hq.json": "COPD1",
            "k_perms_npa_hq.json": "NPA-R",
            "k_perms_ba_hq.json": "Barabási–Albert"
        },
        {
            "k_perms_npa_hq.json": "../../output/network_stats/network_stats.json",
            "k_perms_ba_hq.json": "../../output/ba_stats_03/ba_stats.json"
        }
    )
